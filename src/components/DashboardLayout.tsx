...
import { LayoutDashboard, FileText, Upload, Award, Settings, LogOut, Menu, X, Bell, Briefcase, ClipboardList } from "lucide-react";
...
const roleConfig = {
  ...
  "driving-school": {
    ...
    navItems: [{
      ...
      href: "/driving-school/drivers",
      icon: FileText
    }, {
      label: "Evaluations",
      href: "/driving-school/evaluations",
      icon: Upload
    }]
  },
  "medical-lab": {
    ...
    navItems: [{
      ...
      label: "Assigned Drivers",
      href: "/medical-lab/drivers",
      icon: FileText
    }, {
      label: "Medical Reports",
      href: "/medical-lab/reports",
      icon: Upload
    }]
  },
  "verification-agent": {
    ...
    navItems: [{
      label: "Dashboard",
      href: "/verification-agent",
      icon: LayoutDashboard
    }, {
      label: "Verifications",
      href: "/verification-agent/verifications",
      icon: FileText
    }]
  },
  admin: {
...
const DashboardLayout = ({
  ...
  return <div className="min-h-screen bg-muted/30">
      ...
      {/* Sidebar */}
      <aside className={cn("fixed top-0 left-0 bottom-0 z-40 w-64 bg-background border-r border-border transition-transform duration-300", "lg:translate-x-0", sidebarOpen ? "translate-x-0" : "-translate-x-full")}>
        <div className="flex flex-col h-full">
          ...
          <nav className="flex-1 p-4 space-y-1">
            ...
            return <Link key={item.href} to={item.href} onClick={() => setSidebarOpen(false)} className={cn("flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-colors", isActive ? "bg-primary text-primary-foreground" : "text-muted-foreground hover:bg-muted hover:text-foreground")}>
      ...
      {/* Mobile Overlay */}
      {sidebarOpen && <div className="lg:hidden fixed inset-0 z-30 bg-background/80 backdrop-blur-sm" onClick={() => setSidebarOpen(false)} />}
